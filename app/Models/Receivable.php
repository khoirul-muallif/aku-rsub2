...
{
    protected $fillable = [
        'account_id', 'invoice_number', 'debtor_name',
        'penjamin', 'type', 'invoice_date', 'due_date',
        'paid_date', 'amount', 'paid_amount', 'discount',
    ...
    public static function generateInvoiceNumber(): string
    {
        $year   = now()->year;
        $month  = now()->month;
        $latest = static::where('invoice_number', 'like', "PIU-{$year}{$month}%")
            ->latest('id')->first();
        $next = $latest ? intval(substr($latest->invoice_number, -4)) + 1 : 1;
        return sprintf('PIU-%s%02d-%04d', $year, $month, $next);
    }
